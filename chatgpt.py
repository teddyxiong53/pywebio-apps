...
def get_openai_config():
    ...
    if not openai_config:
        openai_config = input_group('OpenAI API Config', [
            input('API Key', name='api_key', type=TEXT, required=True,value='',
                  help_text='Get your API key from https://platform.openai.com/account/api-keys'),
            input('API Server', name='api_base', type=TEXT, value='https://api.siliconflow.cn/v1', required=True),
        ])
...
def main():
    ...
    """)
    put_select('model', ['Qwen/Qwen1.5-7B-Chat'], label='Model')
